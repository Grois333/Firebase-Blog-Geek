class Post {
  ...
  subirImagenPost (file, uid) {
    ...
    task.on(
      ...
      err => {
        Materialize.toast(`Error uploading file = > ${err.message}`, 4000)
      },
      () => {
        task.snapshot.ref
          .getDownloadURL()
          ...
          .catch(err => {
            Materialize.toast(`Error getting download URL = > ${err}`, 4000)
          })
      }
    )
  }
  ...
  obtenerTemplatePostVacio () {
    return `<article class="post">
      <div class="post-titulo">
          <h5>Create the first Post to the community</h5>
      </div>
      ...
      <div class="post-descripcion">
          <p>Create the first Post to the community</p>
      </div>
  ...
  obtenerPostTemplate (
    ...
    if (imagenLink) {
      return `<article class="post">
            ...
            <div class="post-videolink">
                <a href="${videoLink}" target="blank">Watch video</a>                            
            </div>
            <div class="post-descripcion">
                <p>${descripcion}</p>
            </div>
